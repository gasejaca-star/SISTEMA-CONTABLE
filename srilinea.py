...
# Configuración del Web Service SRI (Capturados con Fiddler)
...
# --- 2. FUNCIONES DE APOYO Y USUARIOS ---
...
def extraer_datos_robusto(xml_file):
    try:
        ...
        subdetalle = " | ".join(items[:5]) if items else "Sin descripción"
...
# --- 6. GENERACIÓN DE EXCEL PROFESIONAL ---
def procesar_a_excel(lista_data):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        f_total = workbook.add_format({'bold': True, 'num_format': fmt_cont, 'border': 1, 'bg_color': '#F3F3F3'})
        
        df.to_excel(writer, sheet_name='COMPRAS', index=False)
        ws = writer.sheets['COMPRAS']
        
        # Ajuste de columnas y formatos
        for i, col in enumerate(df.columns):
            ws.set_column(i, i, 15)
            if i >= 8 and i <= 15: # Columnas numéricas
                ws.set_column(i, i, 12, workbook.add_format({'num_format': fmt_cont}))

        # Aquí podrías añadir tu hoja de "REPORTE ANUAL" con las fórmulas SUMIFS
...
with tab_manual:
    st.header("Procesar Compras")
    up_xmls = st.file_uploader("Selecciona tus XMLs", type=["xml"], accept_multiple_files=True)
    if up_xmls and st.button("GENERAR EXCEL RAPIDITO"):
        res_lista = [extraer_datos_robusto(x) for x in up_xmls if extraer_datos_robusto(x)]
        if res_lista:
            registrar_actividad(st.session_state.usuario_actual, "EXCEL MANUAL", len(up_xmls))
            st.download_button("📥 DESCARGAR EXCEL", procesar_a_excel(res_lista), "Reporte.xlsx")

with tab_sri:
    st.header("Descarga desde SRI")
        ...
        if claves:
            ...
            with zipfile.ZipFile(zip_buffer, "a") as zf:
                for i, cl in enumerate(claves):
                    ...
                    try:
                        r = requests.post(URL_WS, data=env, headers=HEADERS_WS, verify=False, timeout=10)
                        if r.status_code == 200 and "<autorizaciones>" in r.text:
                            zf.writestr(f"{cl}.xml", r.text)
                            datos = extraer_datos_robusto(io.BytesIO(r.content))
