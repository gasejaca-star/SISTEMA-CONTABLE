...
# --- 4. MOTOR DE EXTRACCIÓN XML (CORREGIDO) ---
def extraer_datos_robusto(xml_file):
    try:
        ...
        # Desempaquetar SOAP si es necesario
        ...
        # Detectar Tipo Real con seguridad
        ...
        # CORRECCIÓN IMPORTANTE: Usar la clave "TIPO DE DOCUMENTO" directamente para coincidir con Excel
        base_data = {
            "TIPO": tipo_doc, # Para control interno
            "TIPO DE DOCUMENTO": tipo_doc, # Para Excel
        ...
        if tipo_doc == "RET":
            ...
            # Factor negativo si es Nota de Crédito
            ...
            # Memoria
            if tipo_doc == "NC":
                detalle_final = "" # Vacío para NC como se solicitó
                memo_final = "" # Opcional, vacío para NC
            else:
            ...
            base_data.update({
                ...
                "CONTRIBUYENTE": ruc_cliente # Clave para columna Excel
            })
            return base_data
...
def generar_excel_multiexcel(data_compras=None, data_ventas_ret=None, generar_integral=False):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if data_compras:
            ...
            df_c = df_c[orden_c] # Reordenar
            
            ws_c = wb.add_worksheet('COMPRAS')
            
            # Escribir Cabecera
            for i, c in enumerate(orden_c):
                fmt = f_amar if c in ["OTRA BASE IVA","OTRO IVA","MONTO ICE","PROPINAS","EXENTO DE IVA","NO OBJ IVA","BASE. 0","BASE. 12 / 15","IVA.","TOTAL"] else f_azul
                ws_c.write(0, i, c, fmt)
            
            # Escribir Datos
            for r, row in enumerate(df_c.values, 1):
                for c, val in enumerate(row): 
                    ws_c.write(r, c, val, f_num if isinstance(val, (int,float)) else wb.add_format({'border':1}))

            # --- FILA DE TOTALES COMPRAS (NUEVO) ---
            fila_tot = len(df_c) + 1
            ws_c.write(fila_tot, 0, "TOTAL", f_tot)
            # Sumar columnas numéricas (desde OTRA BASE IVA hasta TOTAL)
            # Indices en orden_c: 9 hasta 18
            for col_idx in range(9, 19): 
                letra = xlsxwriter.utility.xl_col_to_name(col_idx)
                formula = f"=SUM({letra}2:{letra}{fila_tot})"
                ws_c.write_formula(fila_tot, col_idx, formula, f_tot)
            ...
            cols_prof = ["P","Q","R","S"] # Ajustar indices si cambiaron las columnas (Revisemos orden_c)
            # Orden C indices: 
            # 15: Base 0 (P), 16: Base 12 (Q), 17: IVA (R), 18: TOTAL (S)
            # Las bases y el IVA son P, Q, R.
            cols_vals = ["P","Q","R"]

            for r, mes in enumerate(meses):
                fila = r+4; ws_ra.write(r+3,0,mes.title(),f_num)
                f_pr = "+".join([f"SUMIFS('COMPRAS'!${l}:${l},'COMPRAS'!$A:$A,\"{mes}\",'COMPRAS'!$I:$I,\"PROFESIONAL\")" for l in cols_vals])
                ws_ra.write_formula(r+3,1,"="+f_pr,f_num)
                for cidx, cat in enumerate(cats):
                    f_pe = "+".join([f"SUMIFS('COMPRAS'!${l}:${l},'COMPRAS'!$A:$A,\"{mes}\",'COMPRAS'!$H:$H,\"{cat}\")" for l in cols_vals])
        ...
        if data_ventas_ret:
            ...
            for i, c in enumerate(orden_v):
                fmt = f_verd if i >= 12 else f_azul 
            ...
            # --- FILA TOTALES VENTAS ---
            fila_tot_v = len(df_v) + 1
            ws_v.write(fila_tot_v, 0, "TOTAL", f_tot)
            # Sumar columnas numéricas (desde index 7 en adelante aprox)
            for col_idx in range(7, 19):
                letra = xlsxwriter.utility.xl_col_to_name(col_idx)
                formula = f"=SUM({letra}2:{letra}{fila_tot_v})"
                ws_v.write_formula(fila_tot_v, col_idx, formula, f_tot)
...
with tab_main_xml:
        ...
        if up_ventas_ret and st.button("Procesar Ventas y Cruce Retenciones"):
            ...
            if data_raw:
                ...
                if ventas_integradas:
                    st.session_state.data_ventas_cache = ventas_integradas
    ...
    with st3:
        st.write("Genera informe consolidado con datos de las pestañas anteriores.")
        c_ok = len(st.session_state.data_compras_cache) > 0
        v_ok = len(st.session_state.data_ventas_cache) > 0
        st.info(f"Estado: Compras ({'OK' if c_ok else 'Pendiente'}), Ventas ({'OK' if v_ok else 'Pendiente'})")
        ...
        else: st.warning("Procese primero los datos en las pestañas anteriores.")# (El código de las pestañas SRI iría aquí, exactamente como estaba antes)
